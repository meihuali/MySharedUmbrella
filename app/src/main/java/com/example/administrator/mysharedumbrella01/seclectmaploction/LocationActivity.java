...
public class LocationActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    private void initView() {
        ...
        tvSeek = (ImageView) findViewById(R.id.tv_seek);
        ...
        public void onPoiSearched(PoiResult result, int rCode) {
            if (rCode == 1000) {
               ...
               //     StyledDialog.buildLoading("加载中···").show();
                    ...
                    adapter.setOnitembody(new LocationAdapter.onItemBody() {
                        @Override
                        public void onitembody(String text,int postion,String jingdu,String weidu) {
                            ToastUtil.showShortToast(getApplicationContext(),text);
                            Intent mIntent = new Intent();
                            //这里保存 用户 点击的位置信息
                            mIntent.putExtra("change01", text);
                            // 保存用户点击的经度
                            mIntent.putExtra("jingdu",jingdu);
                            //保存用户点击的维度
                            mIntent.putExtra("weidu",weidu);
                            // 设置结果，并进行传送
